...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        scores, a_cache = affine_forward(ar_out, self.params['W2'], self.params['b2'])
        pass
        ...
        dar_out, dw2, db2 = affine_backward(dout, a_cache)
        dx, dw1, db1 = affine_relu_backward(dar_out, ar_cache)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for lay in range(1,self.num_layers):
            if self.normalization == "batchnorm":
                layer_input, ar_cache[lay] = affine_bn_relu_forward(layer_input, 
                                        self.params['W'+str(lay)], self.params['b'+str(lay)], 
                                        self.params['gamma'+str(lay)], self.params['beta'+str(lay)], self.bn_params[lay-1])
        ...
        for lay in range(self.num_layers-1, 0, -1):
            loss += 0.5 * self.reg * np.sum(np.square(self.params['W'+str(lay)]))
            if self.use_dropout:
            ...
            grads['W'+str(lay)] = dw + self.reg * self.params['W'+str(lay)]
            grads['b'+str(lay)] = db
            if self.normalization:

...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        batch_x = (x - running_mean) / np.sqrt(running_var + eps)
        out = gamma * batch_x + beta
...
def batchnorm_backward(dout, cache):
    ...
    dx_jian = gamma * dout
    dxcent = ((batch_var + eps) ** -0.5) * dx_jian
    d1_over_x = np.sum((x - batch_mean) * dx_jian, axis=0)
    droot = -0.5 * ((batch_var + eps) ** -1.5) * d1_over_x
    dsum = np.ones_like(x) / N * droot
    dsquare = 2 * (x - batch_mean) * dsum
    dx2 = dsquare + dxcent
    dsum = -1 * np.sum(dx2, axis=0)
    dx1 = np.ones_like(x) / N * dsum
    dx = dx1 + dx2
...
def batchnorm_backward_alt(dout, cache):
    ...
    N = x.shape[0]
    dx_hat = dout * gamma
    dvar = np.sum(dx_hat* (x - batch_mean) * -0.5 * ((batch_var + eps) ** -1.5), axis = 0)

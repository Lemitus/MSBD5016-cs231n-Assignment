...
def rnn_step_backward(dnext_h, cache):
    ...
    dx = np.dot(dh, Wx.T) # N*H H*D ->N*D
    dWx = np.dot(x.T, dh) # D*N N*H ->D*H
...
def rnn_backward(dh, cache):
    ...
    for i in range(T-1, -1, -1):
        dh0 += dh[:,i,:]
        dxt, dh0, dWxt, dWht, dbt = rnn_step_backward(dh0, cache[i])
        dx[:,i,:] = dxt
        dWx += dWxt
        dWh += dWht
        db += dbt
...
def lstm_backward(dh, cache):
    ...
    for i in range(T-1, -1, -1):
        dh0 += dh[:,i,:]
        dxt, dh0, dc0, dWxt, dWht, dbt = lstm_step_backward(dh0, dc0, cache[i])
        dx[:,i,:] = dxt
        dWx += dWxt
        dWh += dWht
        db += dbt

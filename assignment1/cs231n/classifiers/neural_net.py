...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        loss = -1 * np.sum(np.log(prob[range(N), y]))
        loss = loss / N + reg * np.sum(np.square(W1)) + reg * np.sum(np.square(W2))
        pass
        ...
        temp_score[range(N), y] -= 1
        temp_score /= N
        grads['W2'] = h1.T.dot(temp_score) + 2 * reg * W2
        grads['b2'] = np.sum(temp_score, axis=0)
        
        dh1 = temp_score.dot(W2.T)
        dh1_Relu = (h1 > 0)*dh1
